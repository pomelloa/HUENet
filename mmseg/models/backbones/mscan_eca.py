...
class ECA(nn.Module):
    ...
    def __init__(self, channels,kernel=3):
        ...
        self.conv = nn.Conv1d(
            1, 1, kernel_size=kernel, padding=(kernel - 1) // 2
        )
...
class ECA_MSCAN(BaseModule):
    ...
    def __init__(self,
        ...
        for i in range(num_stages):
            ...
            eca_block = ECA(embed_dims[i])
            norm = nn.LayerNorm(embed_dims[i])
            batch_norm = nn.BatchNorm2d(embed_dims[i])
            cur += depths[i]

            setattr(self, f'patch_embed{i + 1}', patch_embed)
            setattr(self, f'block{i + 1}', block)
            setattr(self, f'eca_block{i + 1}', eca_block)
    ...
    def forward(self, x):
        ...
        for i in range(self.num_stages):
            ...
            eca_block = getattr(self, f'eca_block{i + 1}')
