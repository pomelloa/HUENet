...
class MultiModalGate(nn.Module):
    def __init__(self, in_features):
        super(MultiModalGate, self).__init__()
        self.fc_gate1 = nn.Linear(in_features, in_features)
        self.fc_gate2 = nn.Linear(in_features, in_features)
        self.fc_gate3 = nn.Linear(in_features, in_features)
        self.fc_gate4 = nn.Linear(in_features, in_features)

    def forward(self, x1, x2, x3,x4):
        gate1 = torch.sigmoid(self.fc_gate1(x1))
        gate2 = torch.sigmoid(self.fc_gate2(x2))
        gate3 = torch.sigmoid(self.fc_gate3(x3))
        gate4 = torch.sigmoid(self.fc_gate4(x4))

        output = torch.cat((gate1 * x1, gate2 * x2, gate3 * x3,gate4 * x4), dim=1)
        return output

@MODELS.register_module()
class Gate_MSCAN(BaseModule):
    ...
    def __init__(self,
        ...
        self.mmgate = MultiModalGate(512)
    ...
    def forward(self, x):
        ...
        # print(B)
        outs = []
        modal1 = x[:, 0:3, :, :]  # 选择第 4 (索引 3) 和第 5 (索引 4) 个通道
        modal2 = x[:, 3:5, :, :]  # 模态1特征
        modal3 = x[:, 5:6, :, :]  # 模态2特征
        modal4 = x[:, 6:, :, :]  # 选择通道 5 到 29
        x = self.mmgate(modal1,modal2,modal3,modal4)
        print(x.shape)
